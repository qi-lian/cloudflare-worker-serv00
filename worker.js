  ...
  <body class="dark">
    ...
    <script>
      ...
      async function runScript() {
        ...
        statusDiv.textContent = '脚本运行中...';
        try {
          const response = await fetch('/run', { method: 'POST' });
          if (response.ok) {
            const results = await response.json();
            displayResults(results);
            statusDiv.textContent = '脚本运行成功!';
...
async function handleScheduled(scheduledTime) {
  ...
  for (const account of accounts) {
    const { username, type, panelnum, checkUrl } = account; 

    let baseUrl = type === 'ct8'
      ? 'https://panel.ct8.pl'
      : `https://panel${panelnum}.serv00.com`

    // 使用 checkUrl 或 baseUrl 检查主机是否活动
    const urlToCheck = checkUrl || baseUrl; 
    if (await isHostActive(urlToCheck)) {
      const message = `检测的网址 ${urlToCheck} 处于活动状态，跳过登录和添加 cron 任务`; // 修改提示信息
      console.log(message);
      results.push({
        username,
        type,
  ...
  try {
    ...
    if (loginResponse.status === 302 && loginResponse.headers.get('location') === '/cron/') {
      ...
      for (const cronCommand of cronCommands) {
        // 使用正则表达式来匹配 cron 命令
        const cronCommandRegex = new RegExp(`^${cronCommand.command}\\s`);

        // 检查 cron 命令是否存在，使用两个判断条件
        if (!cronCommandRegex.test(cronListContent) && !cronListContent.includes(cronCommand.command)) { 
          // 访问添加 cron 任务页面
          ...
          // 获取 cron 任务的频率设置
          const { minuteTimeInterval, minute, hourTimeInterval, hour, dayTimeInterval, day, monthTimeInterval, month, dowTimeInterval, dow } = cronCommand;

          const formData = new URLSearchParams({
            'csrfmiddlewaretoken': newCsrfToken,
            'spec': 'manual',
            'minute_time_interval': minuteTimeInterval,
            'minute': minute,
            'hour_time_interval': hourTimeInterval,
            'hour': hour,
            'day_time_interval': dayTimeInterval,
            'day': day,
            'month_time_interval': monthTimeInterval,
            'month': month,
            'dow_time_interval': dowTimeInterval,
            'dow': dow,
            'command': cronCommand.command, // 使用 cronCommand.command 获取命令
          ...
          if (success) {
            if (addCronResponseContent.includes('Cron job has been added') || addCronResponseContent.includes('Zadanie cron zostało dodane')) {
              const message = `添加了新的 cron 任务：${cronCommand.command}`;
              console.log(message);
              ...
              if (checkCronListContent.includes(cronCommand)) {
                const message = `确认添加了新的 cron 任务：${cronCommand.command}`;
                console.log(message);
                await sendTelegramMessage(`账号 ${username} (${type}) ${message}`);
                cronResults.push({ success: true, message });
              } else {
                const message = `尝试添加 cron 任务：${cronCommand.command}，但在列表中未找到。可能添加失败。`;
                console.error(message);
                cronResults.push({ success: false, message });
              }
            }
          } else {
            const message = `添加 cron 任务失败：${cronCommand.command}`;
            console.error(message);
            cronResults.push({ success: false, message });
          }
        } else {
          const message = `cron 任务已存在：${cronCommand.command}`; // 使用 cronCommand.command 获取命令
          console.log(message);
          cronResults.push({ success: true, message });
        }
      }
...
// 检查网站是否活动
async function isHostActive(urlToCheck) {
  try {
    const response = await fetch(urlToCheck); // 使用 fetch 函数访问网站的特定页面或 API
    return response.ok; // 如果响应状态码在 200-299 范围内，则认为网站处于活动状态
  } catch (error) {
    console.error(`检查网站 ${urlToCheck} 是否活动时出错:`, error);
    return false; // 如果出现错误，则认为网站不处于活动状态
  }
}
